...
def is_ind(len_sepal, width_sepal):
    ...
    table = pd.crosstab(df['lenght_of_petal'], df['width_of_sepal'])
    ...
    for i in range(5):
        for j in range(5):
            s += np.power(table.iloc[j][i], 2) / (table.iloc[5][i] * table.iloc[j][5])
    hi_prac = (s - 1) * 50
    hi_teor = 26.3
...
def main():
    ...
    width_petal = df['lenght_of_petal']
    ...
    if T > T_table:
        print("Коэффициент корреляции значим, гипотеза принимется\n\n")
    else:
